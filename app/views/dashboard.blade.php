                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title">{{trans('dashboard.notif_install');}}</h4>
                        </div>
                        <div class="modal-body">
                            <p><?php echo $message; ?></p>
                        </div>
                        <div class="modal-footer">
                            <a href="{{ URL::Route('AdminSettingDontShow') }}"><button type="button" class="btn btn-default" >{{trans('dashboard.not_show_again');}}</button></a>
                            <button type="button" class="btn btn-default" data-dismiss="modal">{{trans('dashboard.close');}}</button>
                            <a href="{{ URL::Route('AdminSettingInstallation') }}"><button type="button" class="btn btn-primary">{{trans('dashboard.change_now');}}</button></a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="box-header">
                <div class="inner">
                    <h2 style="padding-left:1em; padding-top:1em; padding-bottom:1em">
                        {{trans('adminController.hi_user')}},
                        <br>
                        {{trans('adminController.use_menu_system')}}
                    </h2>
                </div>
            </div>
        </div>
    ...
    <div class="col-lg-4 col-xs-6">
        ...
        <div class="small-box bg-aqua">
            <div class="inner">
                ...
                <p>
                     {{trans('dashboard.total_trip_conf');}}
    ...
    <div class="col-lg-4 col-xs-6">
        ...
        <div class="small-box bg-green">
            <div class="inner">
                ...
                <p>
                    {{ trans('dashboard.completed_rides');}} 
                </p>
            </div>
            <div class="icon">
    ...
    <div class="col-lg-4 col-xs-6">
        ...
        <div class="small-box bg-red">
            <div class="inner">
                ...
                <p>
                    {{trans('dashboard.cancelled_rides');}}
                </p>
            </div>
            <div class="icon">
    ...
    <div class="col-lg-4 col-xs-6">
        ...
        <div class="small-box bg-red">
            <div class="inner">
                ...
                <p>
                    {{ trans('dashboard.Schedules_Total'); }}
                </p>
            </div>
            <div class="icon">
                <?php /* $icon = Keywords::where('keyword', 'completed_trip')->first(); */ ?>
...
<div class="box box-danger">
        ...
        <div class="row">
            ...
            <form role="form" method="get" action="{{ URL::Route('AdminReport') }}">
                ...
                <div class="col-md-4 col-sm-4 col-lg-4">
                    <input type="text" class="form-control" style="oveerflow:hidden;" id="request_id" name="request_id" value="{{ Input::get('request_id') }}" placeholder="{{trans('dashboard.request_id');}}">
                    <br>
                </div>

                <!-- filtro de data de inicio do intervalo --> 
                <div class="col-md-4 col-sm-4 col-lg-4">
                    <input type="text" class="form-control" style="oveerflow:hidden;" id="start-date" name="start_date" value="{{ Input::get('start_date') }}" placeholder="{{trans('dashboard.start_date');}}">
                    <br>
                </div>
                ...
                <div class="col-md-4 col-sm-4 col-lg-4">
                    ...
                    <select name="status"  class="form-control">
                        <option value="0"> {{trans('dashboard.status');}}</option>
                        <option value="1" <?php echo Input::get('status') == 1 ? "selected" : "" ?> >{{trans('dashboard.completed');}}</option>
                        <option value="2" <?php echo Input::get('status') == 2 ? "selected" : "" ?>>{{trans('dashboard.cancelled');}}</option>
                    </select>
                    <br>
                </div>
                ...
                <div class="col-md-12 col-sm-12 col-lg-12">
                    <select name="pay_status"  class="form-control">
                        <option value="0"> {{trans('dashboard.pay_status');}}</option> 
                        <option value="1" <?php echo Input::get('pay_status') == 1 ? "selected" : "" ?> >{{trans('dashboard.completed');}}</option>
                        <option value="2" <?php echo Input::get('pay_status') == 2 ? "selected" : "" ?> >{{trans('dashboard.request_cancelled');}}</option>
                        <option value="3" <?php echo Input::get('pay_status') == 3 ? "selected" : "" ?> >{{trans('dashboard.pending_cancel_fee');}}</option>
                        <option value="4" <?php echo Input::get('pay_status') == 4 ? "selected" : "" ?>>{{trans('dashboard.pending');}}</option>
                    </select>
                    <br>
                </div>

                <!-- filtro de metodo de pagamento -->       
                @if(Settings::countPaymentMethods() > 1)         
                    <div class="col-md-12 col-sm-12 col-lg-12">           
                        ...
                        @if(Settings::getPaymentCard() == 1)
                            ...
                            <label for="payment_card">{{trans('setting.card');}}</label>
                        @endif
                        ...
                        <label for="payment_money">{{trans('setting.money');}}</label>
                        @endif
                        
                        @if(Settings::getPaymentVoucher() == 1)
                        ...
                        <label for="payment_voucher">{{trans('setting.voucher');}}</label>
                        <br>
                        @endif
    ...
    <div class="box-footer">
        <button type="submit" name="submit" class="btn btn-primary" value="Filter_Data">{{trans('dashboard.filter');}}</button>
        <button type="submit" name="submit" class="btn btn-primary" value="Download_Report">{{trans('dashboard.down_report');}}</button>
    </div>
    ...
    <table class="table table-bordered">
        <tbody>
            <tr>
                ...
                <!-- nome do usuario -->
                <th>{{trans('dashboard.name_conf');}} {{ trans('customize.User');}}</th>
                ...
                <!-- metodo de pagamento -->
                <th>{{trans('dashboard.payment_method');}}</th>

                <!-- status da solicitacao -->
                <th>{{trans('dashboard.status');}}</th>

                <!-- status do pagamento -->
                <th>{{trans('dashboard.pay_status');}}</th>

                <!-- valor bruto -->
                <th>{{trans('dashboard.gross_value');}}</th>

                <!-- desconto promocional -->
                <th>{{trans('dashboard.promo_discount');}}</th>

                <!-- taxas -->
                <th>{{trans('dashboard.taxes');}}</th>

                <!-- valor do prestador -->
                <th>{{trans('dashboard.provider_value');}}</th>

                <!-- valor da empresa -->
                <th>{{trans('dashboard.company_value');}}</th>
            </tr>
        ...
        <label>
            <tr style="font-weight: bold;">
                <td colspan="8">  {{trans('dashboard.total');}} </td>
                <td>
                    <?= $currency_sel ?>
