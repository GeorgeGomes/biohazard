...
<h2>{{trans('website.bank_detail_add');}}</h2>
<hr>
  <div class="box box-primary">
    <div class="box-header">
  	...
  	<form method="post" action="{{ route('ProviderSBanking') }}" id="payment-form" enctype="multipart/form-data">
        ...
        <input type="text" name="first_name" class="form-control" placeholder="{{ trans('provider.first_name');}" value="{{ $provider_first_name }}" required><br>
        <input type="text" name="last_name" class="form-control" placeholder="{{trans('provider.last_name');}}" value="{{$provider_last_name }}" required><br>
        <input type="text" size="20" data-stripe="country" id="country" class="form-control" value="US" placeholder="{{trans('user.pay_method');}}" required /><br>
        <input type="text" size="20" data-stripe="accountNumber" id="accountNumber" placeholder="{{trans('provider.account_number');}}" class="form-control" name="account_number" required /><br>
        <input type="text" size="40" data-stripe="routingNumber" id="routingNumber" class="form-control" placeholder="{{trans('provider.routing_number');}}" name="routing_number" required /><br>
        <select name="type" class="form-control">
          <option value="individual">{{trans('provider.individual');}}</option>
          <option value="corporate">{{trans('provider.coorporative');}}</option>
        </select><br>
        <input type="text" name="email" class="form-control" placeholder="{{trans('provider.mail_grid');}}" value="{{$provider_email }}" required><br>
        <br><input type="submit" value="{{ trans('website.update_change');}}" class="btn btn-block btn-flat btn-success">
  		</div>
  	</form>
  </div>
    </div>
