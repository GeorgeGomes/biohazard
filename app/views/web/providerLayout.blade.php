			...
			<aside>
				<div id="sidebar"  class="nav-collapse ">
					...
					<ul class="sidebar-menu" id="nav-accordion">
						...
						<p class="centered"><a href="{{ URL::Route('ProviderTrips') }}"><img src="<?= Session::get('provider_pic') ? Session::get('provider_pic') : asset_url() . '/web/default_profile.png' ?>" class="img-circle" width="60"></a></p>
						<h5 class="centered">{{ Session::get('provider_name') }}</h5>
						@if ( strcmp(Session::get('status'), "APROVADO") == 0 )
							<p  class="centered"><a href="{{ URL::Route('providerDocuments') }}"><span class="label label-success">{{trans('provider.approved_grid')}}</span></a></p>
						@elseif ( strcmp(Session::get('status'), "REJEITADO") == 0 )
							<p  class="centered"><a href="{{ URL::Route('providerDocuments') }}"><span class="label label-danger">{{trans('provider.rejected_grid')}}</span></a></p>
						@elseif ( strcmp(Session::get('status'), "EM_ANALISE") == 0 )
							<p  class="centered"><a href="{{ URL::Route('providerDocuments') }}"><span class="label label-warning">{{trans('provider.analysis_grid')}}</span></a></p>
						@else
							<p  class="centered"><a href="{{ URL::Route('providerDocuments') }}"><span class="label label-default">{{trans('provider.suspended_grid')}}</span></a></p>
						@endif
						
						<li class="mt">
							<a href="{{ URL::Route('ProviderTrips') }}">
								<i class="fa fa-car"></i>
								<span>{{trans('user_provider_web.my_trip')}}</span>
							</a>
						</li>
						...
						<li class="" id="flow21">
							...
							<a href="{{ URL::Route('providerProfile') }}">
								...
								<span>{{trans('user_provider_web.profile')}}</span>
							</a>
						</li>
                        <li class="" id="flow21">

                            <a href="{{ URL::Route('providerBankAccount') }}">
                                <i class="fa fa-bank"></i>
                                <span>{{trans('user_provider_web.bank_account')}}</span>
                            </a>
                        </li>       
						<li class="" id="flow23">
							<a href="{{ URL::Route('providerDocuments') }}">
								<i class="fa fa-file"></i>
								<span>{{trans('user_provider_web.document')}}</span>
							</a>
						</li>
						
						<li class="">
							<a href="{{ URL::Route('ProviderPricePolicy') }}">
								<i class="fa fa-money"></i>
								<span>{{ trans('user_provider_web.price_policy'); }}</span>
							</a>
						</li>
						...
						<li class="">
							<a href="{{ URL::Route('providerTripInProgress') }}">
								...
								<span>{{trans('user_provider_web.trip_now')}}</span>
							</a>
						</li> 
						...
						<li class="">
							<a href="{{ URL::Route('ProviderRequests_payment') }}">
			...
			*********************************************************************************************************************************************************** -->
			...
			<footer class="site-footer">
				<div class="text-center">
					{{ trans('email.Pifou'); }}. {{ trans('website.contact_info'); }}
					<a href="#" class="go-top">
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title">{{trans('user_provider_web.new_request')}}</h4>
					</div>
					<div class="modal-body">
						<p>{{trans('user_provider_web.new_trip_request')}}&hellip;</p>
						<div style="position:relative;float:left">
							<img id="user-image" src="" class="img-circle" width="60px">
						</div>
						<div style="position:relative;float:left;left:20px;">
							<b id="user-name"></b><br>
							<i>{{trans('user_provider_web.rate')}} - </i><b id="user-rating"> 3 </b><b>/ 5</b>
					...
					<div class="modal-footer">
						<a href="" id="accept-url"><button type="button" class="btn btn-success">{{trans('user_provider_web.accept')}}</button></a>
						<a href="" id="decline-url"><button type="button" class="btn btn-danger">{{trans('user_provider_web.reject')}}</button></a>
					</div>
				</div><!-- /.modal-content -->
			</div><!-- /.modal-dialog -->
		</div><!-- /.modal -->
			...
			function successFunction(position) {
				...
				$.ajax({
					...
					success: function (msg) {
						console.log("{{trans('user_provider_web.Location_captured')}}");
					},
					processData: false,
				});
			}
