...
<!-- app/views/providertypecategory/edit.blade.php -->
...
{{ Form::model($providertypecategory, array('route' => array('admin.providertypecategory.update', $providertypecategory->id), 'method' => 'PUT')) }}
	...
	<div class="form-group">
		<label>{{trans('providertypecategory.services')}}</label> </br>
			@foreach($services as $service)
				<input class="icheckbox_minimal" <?=($service->hasAssociationByCategoryId($providertypecategory->id) ? 'checked' : '')?> id="services" name="services[]" type="checkbox" value="{{$service->id}}">
				{{ $service->name}}
			@endforeach
	</div>

	{{ Form::submit(trans('customize.edit'), array('class' => 'btn btn-primary right')) }}
