...
class WebController extends \BaseController {
    ...
    public function termsncondition() {
        ...
        return View::make('website.termsandconditions')
                        ->with('title', trans('website.terms_and_conditions'))
                        ->with('logo', $logo)
        ...
        if ($provider->merchant_id == NULL) {
            if (Config::get('app.default_payment') == 'stripe') {
                return View::make('website.banking_provider_stripe')
                                ->with('title', trans('user_provider_web.bank_provider'))
                                ->with('logo', $logo)
                                ->with('favicon', $favicon)
                                ->with('app_name', $app_name)
    ...
    public function providerB_bankingSubmit() {
        ...
        $result = Braintree_MerchantAccount::create(
                        array(
                            ...
                            'id' => trans('email.Pifou') . Input::get('id')
                        )
        );
    ...
    public function page($url_key) {
        ...
        $information = Information::where('url_key', '=',$url_key)->first();

        return View::make('website.page')
                        ->with('title', $information->title)
                        ->with('logo', $logo)
                        ->with('favicon', $favicon)
                        ->with('app_name', $app_name)
    ...
    public function track_ride() {
        ...
        if ($request) {
            ...
            $app_name = Config::get('app.website_title');
            // walk location
            $reqloc = RequestLocation::where('request_id', $request->id)->first();
    ...
    public function get_track_loc($id) {
        ...
        if ($request) {
            ...
            $app_name = Config::get('app.website_title');
            // walk location
            $start_loc = RequestLocation::where('request_id', $request->id)->first();
            $reqloc = RequestLocation::where('request_id', $request->id)->orderBy('id', 'desc')->first();
            $title = trans('adminController.track') . ' ' . $user_name;
